...
const initialValues = { password: '', username: '', repeat_password: '', email: '', accepted_terms: false };

// Used until appConfig resolves; these match the URLs the rest of the app opens.
const FALLBACK_TERMS_URL = 'https://dazzzle.org/terms-of-use';
const FALLBACK_PRIVACY_URL = 'https://dazzzle.org/privacy-policy';
...
const SignUp = () => {
    ...
    // The success sheet renders outside Formik, so the resend handler can't read
    // values.email. Captured from the submitted values rather than per-keystroke:
    // a ref avoids re-rendering the form on every character, and reading it at
    // submit also covers autofill, which never fires onChangeText.
    const emailRef = useRef('');

    const termsUrl = appConfig?.terms_and_conditions_url || FALLBACK_TERMS_URL;
    const privacyUrl = appConfig?.privacy_policy_url || FALLBACK_PRIVACY_URL;

    const createAccount = useCallback(async (formValues: SigUpForm) => {
        try {
            show();
    ...
    const handleResendEmail = useCallback(async () => {
        try {
            show();
            const response: any = await axiosRequest.post('/user/process-resend-activation-mail', { email: emailRef.current });
            hide();
            if (response.reaction === ReactionCodes.SUCCESS) {
    ...
    const openTerms = useCallback(() => openLink(termsUrl), [termsUrl]);
    const openPrivacy = useCallback(() => openLink(privacyUrl), [privacyUrl]);
                                ...
                                <View className='my-5 space-y-3'>
                                    ...
                                    <View className='flex flex-row items-center py-2.5 gap-2'>
                                        ...
                                        <View className='flex flex-wrap flex-1 flex-row gap-1'>
                                            ...
                                            <TouchableHighlight onPress={openTerms}>
                                            ...
                                            <TouchableHighlight onPress={openPrivacy}>
