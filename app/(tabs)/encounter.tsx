...
const EncounterScreen = () => {
  ...
  useEffect(() => {
    if (route.params?.actionUserId && route.params?.actionType) {
        // If we returned with an action
        const { actionUserId, actionType } = route.params;
        const currentUser = users[topCardIndex];
        
        // Clear params to avoid loop
        navigation.setParams({ actionUserId: null, actionType: null });

        if (currentUser && currentUser.id === actionUserId) {
            // Trigger animation
            setTriggerSwipeDirection(actionType === 'like' ? 'right' : 'left');
        }
    }
  }, [route.params]);
