...
const BlockedUsers = () => {
  const { axiosRequest } = useAxiosContext();
     ...
     const fetchLikedUsers = async (pageUrl = '/blocked-users-list', hideLoader = true) => {
         try {
             setLoading(true);
     ...
     const refreshUsers = async () => {
         try {
             setRefreshing(true);
             const { data } = await axiosRequest.get('/blocked-users-list', { headers: { 'hide-loader': 'true' } });
             if (data.reaction === ReactionCodes.SUCCESS) {
                 const { usersData, totalCount, nextPageUrl } = data.data;
                 setUsers(usersData);
                 setPaginationDetails({ totalCount, nextPageUrl });
     ...
     useEffect(() => {
         fetchLikedUsers('/blocked-users-list', false);
     }, []);
     ...
     return (
         <View className='bg-[#1A1A1A] h-full'>
             <FlatList
                 className='p-1'
                 data={users}
                 keyExtractor={(item, index) => item._uid}
